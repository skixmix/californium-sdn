...
public class BlockwiseClientSideTest {
	...
	public void testRequestIsCancelledIfBodyExceedsMaxBodySize() throws Exception {
		...
		respPayload = generateRandomPayload(MAX_RESOURCE_BODY_SIZE + 10);
		String path = "test";
		Request request = createRequest(GET, path, server);

		client.sendRequest(request);
		server.expectRequest(CON, GET, path).storeBoth("A").go();
		server.sendResponse(ACK, CONTENT).loadBoth("A").size2(respPayload.length()).block2(0, true, 128).payload(respPayload, 0, 128).go();
